...
public class BlogCategoryServlet extends HttpServlet {
	static final Logger logger = LogManager.getLogger(BlogCategoryServlet.class.getName());
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (!StringUtils.isEmpty(category)) {
			...
			BlogSearchResult result = blogService.getAllBlogByCategory(category, pageValue, sizeInt);
			List<BlogVo> blogVos = result.getBlogVos();
			Long blogSize = result.getTotalElement();

			request.setAttribute("blogsByCategory", result);
